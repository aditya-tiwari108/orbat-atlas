...
Production deployment `dpl_5Whe4MK3kVm66c2R4qFB3y15XYTT` (application commit `526b3fe`) is live at https://orbat-atlas.vercel.app. All 22 scenarios passed against the production alias on 13 September 2026. A separate screenshot/network check of the NCC overview, 4 Kar Eng Coy and Army zoom-out recorded 52 successful CARTO responses, no browser exceptions and no map-error messages. Rectangular tile-loading edges seen in early captures disappeared after network activity settled; the settled screenshots were reviewed.

GitHub validation also passed for the application commit: https://github.com/aditya-tiwari108/orbat-atlas/actions/runs/34718301347.

## Leadership panels and command-map integration — 14 September 2026

Service home views now open a dismissible leadership panel. Army, Navy and Air Force show their service chief alongside the CDS; NCC shows its Director General. A shared portrait component provides larger images and names. Selected command dossiers retain a short overview, Wikipedia link, hierarchy navigation and a single accuracy line at the bottom. Detailed field evidence remains in the dataset and coverage report; photo attribution is linked from About through the generated image-credit page.

Training and maintenance command HQs now participate in the same map layer as the other commands: seven Army commands, four entries in Navy including the tri-service command, and seven Air Force commands. Camera padding and the label-layout viewport reserve space for the home panel without recreating the map when the panel is toggled. No territorial coverage is inferred for training or maintenance commands.

Five home-leadership photographs were added, together with Western Air Command's official portrait and Northern Command's official speaker photograph. The Northern source supplies only a low-resolution image. ARTRAC's newly reported commander still has no verified reusable portrait; NCC directorate portrait gaps remain explicit in the coverage report. Every image is associated with a named officer and its source/rights record. Appointment dates and current role labels do not assert the photograph was taken in that appointment.

Local production build, lint, catalog/evidence/cartography tests and all 25 Playwright scenarios passed. The new cases check every command's image or unavailable state, large photo dimensions, correct service chiefs/CDS, service switching, home-panel dismissal/reopening, training/maintenance map selection, Wikipedia links and the single bottom note. Existing desktop zoom, NCC boundaries, 4 Kar Eng Coy, shared HQs, keyboard, browser history, map failure and mobile smoke scenarios remain covered. Rendered contact sheets of all 17 available command photographs and all four home views were manually reviewed; three event-photo crops and three formal portrait frames were corrected after that review.

Production deployment `dpl_8yeZJnhzQ7M41jsMZwSvK7gKvJzA` serves application commit `b097685` at https://orbat-atlas.vercel.app. The live run passed 24 scenarios; the HQ-anchor test initially expired during lazy map-module loading before any map markers existed. Its initial readiness wait was increased from 5 to 20 seconds, leaving the position and zoom assertions unchanged. That scenario and the home-leadership screenshot check then passed against production. All 25 scenarios therefore passed across these runs. Home screenshots now wait for the map layer and network activity to settle. The settled production screenshots were visually reviewed. GitHub CI passed all checks for the application commit: https://github.com/aditya-tiwari108/orbat-atlas/actions/runs/34873637223.

## 15 September — Navy / Air Force detail

Added 104 organizations: 40 flying squadrons, 23 air stations, 21 ships, 19 naval shore establishments and the 1st Training Squadron. Existing ship links are retained; four Eastern Fleet associations now carry explicit PIB membership evidence. Squadron base and command associations have separate evidence, and aircraft/ship-class fields are searchable. Base markers are highlighted when selecting an unlocated squadron; ships remain unlocated. Long child lists expand and filter in the dossier.

The full local Playwright suite passed all 28 cases. New scenarios cover command → base → squadron, aircraft and nickname discovery, direct links, browser Back, fleet expansion/filtering, unlocated ships and the joint command's installations. Existing Army/NCC zoom, anchors, map failures, keyboard access and mobile smoke checks also passed. Final base highlighting was followed by the affected navigation/zoom checks. Build, lint and all four structural test files passed. Desktop screenshots at 1440×900 were reviewed for Eastern Naval Command, Western Fleet, Western Air Command and No. 17 Squadron. Detailed evidence and excluded conflicting records are in the [field audit](navy-airforce-audit.md); validation does not certify a complete current order of battle.

Production verification completed on 15 September 2026: deployment `dpl_ApmNdvkESnkmuJzuWDuTEavD7PgT` serves application commit `097220e8eb811f83c3078f7e0366c2526bba1d49` at https://orbat-atlas.vercel.app. All 28 Playwright scenarios passed against the live site in one run (1.7 minutes), including the new Navy/Air Force flows and all existing regression checks. GitHub validation also passed: https://github.com/aditya-tiwari108/orbat-atlas/actions/runs/34940984871.

Settled production screenshots of No. 17 Squadron and Western Fleet were visually reviewed at 1440×900. The squadron's home-base highlight, aircraft facts, command link and ship-list filtering are present. The published field audit returns HTTP 200 and contains the base/aircraft register; no uncaught page errors were recorded in this review.
